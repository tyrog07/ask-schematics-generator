...
function findFileByName(file, path, host) {
  ...
  throw new schematics_1.SchematicsException(
    `File ${file} not found in ${path} or one of its anchestors`
  );
}
function createAddInjectionContext(options, host) {
  let appComponentFileName = findFileByName(
    "app.component.ts",
    options.path || "/",
    host
  );
  ...
  let serviceFileName = core_1.join(
    core_1.normalize(destinationPath),
    `${dasherize(options.name)}.service`
  );
  let relativeServiceFileName = find_module_1.buildRelativePath(
...
function buildInjectionChanges(context, host, options) {
  ...
  if (!text)
    throw new schematics_1.SchematicsException(
      `File ${options.module} does not exist.`
    );
...
function addConstructorArgument(context, ctorNode, options) {
  ...
  let parameterListNode = siblings.find(
    (n) => n.kind === ts.SyntaxKind.SyntaxList
  );
  if (!parameterListNode) {
    throw new schematics_1.SchematicsException(
      `expected constructor in ${context.appComponentFileName} to have a parameter list`
    );
  }
  let parameterNodes = parameterListNode.getChildren();
  let paramNode = parameterNodes.find((p) => {
    let typeNode = findSuccessor(p, [
  ...
  if (!paramNode && parameterNodes.length == 0) {
    let toAdd = `private ${camelize(context.serviceName)}: ${classify(
      context.serviceName
    )}`;
    return new change_1.InsertChange(
      context.appComponentFileName,
...
function createConstructorForInjection(context, nodes, options) {
  ...
  if (classIdentifierNode.getText() !== "AppComponent") {
    throw new schematics_1.SchematicsException(
      `expected first class in ${context.appComponentFileName} to have the name AppComponent`
    );
  }
  let curlyNodeIndex = siblings.findIndex(
    (n) => n.kind === ts.SyntaxKind.FirstPunctuation
  );
  siblings = siblings.slice(curlyNodeIndex);
  let listNode = siblings.find((n) => n.kind === ts.SyntaxKind.SyntaxList);
  if (!listNode) {
    throw new schematics_1.SchematicsException(
      `expected first class in ${context.appComponentFileName} to have a body`
    );
  }
  let toAdd = `
  constructor(private ${camelize(context.serviceName)}: ${classify(
    context.serviceName
  )}) {
